...
[docs/BENCHMARKS.md](docs/BENCHMARKS.md) is generated by `scripts/bench.py` on the machine that ran it — nothing is typed in by hand. It reports p95 alongside the mean, because a captioner with a 200 ms mean and a 3 s p95 feels broken.

For numbers on real Snapdragon silicon without owning the hardware, `scripts/aihub_profile.py` submits the graphs to the **Qualcomm AI Hub device farm** and writes `docs/AIHUB.md` with a public job link per row. A reviewer can open any of them and see Qualcomm's own measurement of these graphs on their own device.
...
.\.venv\Scripts\python.exe -m pytest        # 84 tests, no weights required
```
